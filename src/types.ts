...
/**
 * Ein gespeicherter Probeprüfungs-Durchlauf (pro Modul + Bereich).
 * Note und laufende Nummer werden im Frontend daraus abgeleitet
 * (nicht gespeichert), damit sie immer konsistent bleiben.
 */
export interface ExamRun {
  id: string; // stabile ID pro Durchlauf (für Sync/Dedup)
  takenAt: string; // ISO-Zeitstempel – bestimmt Reihenfolge & Nummer
  avg: number; // 0–100, Gesamtdurchschnitt
  durationSec: number; // benötigte Zeit in Sekunden
  taskCount: number; // Anzahl Aufgaben im Durchlauf
}
